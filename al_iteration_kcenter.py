...
from deepchem.models.torch_models import CGCNNModel
...
def parse_args():
    ...
    parser.add_argument("--explore", type=int, require=True)
    parser.add_argument("--exploit", type=int, require=True)
    parser.add_argument("--model_path", type=str,  default = 'models/')
    parser.add_argument("--label_path", type=str, default = 'datasets/iter_file/')
    parser.add_argument("--unlabel_path", type=str, default = 'datasets/MXene_dataset.csv')
    parser.add_argument("--cif_path", type=str, default = 'datasets/cif_file/CONTCAR_')
...
class K_sampler(object):
  def __init__(self, args, total_data_num, init_ids=None):
    self.args = args
  ...
  def query(self, inputs, prediction = None):
  ...
  def _top_query(self, predictions, query_number):
    ...
    self.label_ids = np.sort(np.concatenate([self.label_ids, new_batch_ids]))
    self.data_ids = np.delete(self.data_ids, query_ids)
    return new_batch_ids


  def _k_center_query(self, inputs, query_number):
    time0 = time.time()
    ...
    core_embeddings = inputs[
        self.label_ids]  # core point is the data already choosen
    min_dist = 1e5 * torch.ones(self.total_data_num).to(
        un_embeddings.device)
    min_dist[self.label_ids] = 0
...
def training_loop(args):
  label_df = pd.read_csv(args.label_path+'Iter'+str(args.iteration-1)+'_list.csv')
  ...
  sampler = K_sampler(args = args, total_data_num = len(input_data), init_ids=init_id)
  ...
  new_batch_ids = sampler.query(all_emb, all_pred)
